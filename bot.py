...
ADMIN_IDS = {int(id_) for id_ in os.getenv('ADMIN_IDS', '903171487563923499').split(',') if id_.strip()}
...
async def remove_admin(ctx, user: discord.User):
    ...
    if ctx.author.id != 903171487563923499:  # Only the owner can remove admins
...
async def vps_stats(ctx, vps_id: str):
    ...
    try:
        token, vps = bot.db.get_vps_by_id(vps_id)
        if not vps or (vps["created_by"] != str(ctx.author.id) and not is_admin(ctx)):
...
async def set_container_limit(ctx, max_limit: int):
    ...
    if ctx.author.id != 903171487563923499:  # Only the owner can set limit
...
async def reinstall_bot(ctx):
    ...
    if ctx.author.id != 903171487563923499:  # Only the owner can reinstall
